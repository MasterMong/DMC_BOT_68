...
test('CDP: Go to school info page', async ({ cdpPage }) => {
  ...
  if (await cdpPage.getByRole('link', { name: '- ดาวน์โหลดรายชื่อนักเรียน (สร้างวันละครั้ง เวลา 2:00 น.)' }).isVisible()) {
    ...
    // do login flow here
    await cdpPage.goto(`${dmcPortalUrl}/student/38739:36022006/edit`);
    await cdpPage.waitForLoadState('networkidle');
    ...
    await cdpPage.waitForLoadState('networkidle');
    await cdpPage.locator('#scoutId-select').selectOption('1');
    await cdpPage.locator('#redcrossyouthId-select').selectOption('2');
  } else {
